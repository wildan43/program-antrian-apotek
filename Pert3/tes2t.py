...
class linkedlist:
    ...
    def remove(self,y):
        #cari dulu posisi y
        #kalo ada, ya hapus
        #kalo ngga, ya laporkan bahwa penghapusan tidak dapat dilakukan
        if self.len()== 0:
            print("Linkedlist masih kosong")
        elif y == self.headval.dataval: #khusus hapus node pertam (depan)
            target = self.headval
            self.headval = self.headval.nextval
            del target
        else:     
            qi = ka = self.headval
            while ka != None and ka.dataval != y:
                qi = ka
                ka = ka.nextval
            if ka == None:
                print("Node yang ingin diremove",y,"ga ada")
            else:    
                qi.nextval = ka.nextval
                del ka     
'''
